...
export class AppComponent implements OnInit {
  ...
  async SendSavePDF() {
    ...
    // this.sendFile = await html2pdf().from(element).set(opt).outputPdf();

    this.sendFile = await html2pdf().from(element).set(opt).toPdf().output('datauristring');

    // var anchor = document.createElement('a');
    // anchor.href = this.sendFile;
    // anchor.download = 'file.pdf';
    // anchor.click();
    
    // let file = new File([this.sendFile], "someThing");
    // New Promise-based usage:
    // html2pdf().from(element).set(opt).save();
    this.downloadPDF = false;
    this.base64Converted = false;
  ...
  SendPdf() {
    this._appService.sendPdf('Akshay', this.LoginMobile, 'tonyguy.akki@gmail.com', this.sendFile)
      .subscribe((res: any) => {
        console.log(res)
      },
  ...
  showSlideLogin() {
    ...
    this.regForm.controls['PushID'].setValue('334343434343');
    this.changeDetector.detectChanges();
    this.prepareLogin();
    return false;
  }
